  ...
  return (
    <motion.div className="relative h-[90%] p-12 pt-12 rounded-2xl bg-white dark:bg-[#262626] overflow-y-scroll sm:w-[60%]" layoutId={currentId+""} onClick={(e) => e.stopPropagation()}>
      ...
      <div className="max-w-screen-sm m-auto mb-16 border-neutral-200 dark:border-darkMiddleGray100 border-8 rounded-2xl">
        <ImgSlide settings={settings} imgArray={details?.src as unknown as string[]} alt="projectImg" width={600} height={350} className="m-auto"/>
      </div>
      
      <div className="flex flex-col items-start">
        <div className="mb-6">
          {/* 프로젝트 제목, 기간, 기술스택 */}
          <h1 className="text-[26px] font-semibold mb-[2px]">{details?.title}</h1>
          ...
          <div className="flex flex-col gap-y-1 mb-4">
            <div className="flex flex-row items-center">
              ...
              <span className="cursor-pointer underline underline-offset-4">{details?.url}</span>
            </div>
            <div className="flex">
              ...
              <span className="cursor-pointer underline underline-offset-4">{details?.github}</span>
            </div>
          </div>

          {/* 라이브러리, 팀구성, 나의 기여 */}
          <div className="space-y-1">
            {details?.library && <div>사용한 라이브러리 : <span>{details?.library}</span></div>}
            {details?.team && <div>팀구성 : <span>{details?.team}</span></div>}
            {details?.myRole && <div>나의 기여 : <span>{details?.myRole}</span></div>}
          </div>
        </div>
