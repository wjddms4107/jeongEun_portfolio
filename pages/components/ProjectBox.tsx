...
export default function ProjectBox({isClicked, onClick, details, layoutId}: IProjectBoxProps) {
  ...
  return (
    ...
    >
      {isClicked ? (
        <div onClick={()=>handleOpenBox(details?.id)} className="rounded-2xl h-full bg-neutral-100 dark:bg-darkMiddleGray200">
          <Image
            src={details?.src[0]}
            alt={details?.alt}
          ...
          <div className="h-[auto] flex flex-col justify-between p-4 pt-2 overflow-y-scroll  sm:overflow-y-hidden">
            <div className="">
              <div className="flex items-center justify-between">
                ...
                <button onClick={()=>handleOpenBox(details?.id)} className="text-base text-middleGray200 dark:text-middleGray900 hover:text-middleGray300 hover:dark:text-darkMiddleGray900 flex items-center">
                  <span>자세히보기</span>
            ...
            <div className="sm:overflow-y-auto">
              <div className="text-[13px] text-middleGray900 dark:text-darkMiddleGray900 mb-[6px] sm:text-[17px]">
                📌 github : <Link href={details?.github} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="cursor-pointer hover:underline underline-offset-4">{details?.github}</Link>
              </div>
              <div className="text-[13px] text-middleGray900 dark:text-darkMiddleGray900 sm:text-[17px]">
                📌 url : <Link href={details?.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="cursor-pointer hover:underline underline-offset-4">{details?.url}</Link>
              </div>
      ...
      ) : <div className="h-full flex items-center justify-center text-deepGray dark:text-darkMiddleGray300 text-xl font-semibold text-center p-2">{details?.title}</div>}
    </motion.div>
    <AnimatePresence>
      {currentId ? (
            <motion.div
