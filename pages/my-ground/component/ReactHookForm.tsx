...
export default function ReactHookForm() {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full">
      ...
      >
          <div className="mb-2">react-hook-form은 큰 form을 가지고 있어서 유효성 검사를 할 때 너무 편리하다.</div>
          <div>
            register은 onChange, value 등 전부 다 해결해준다. 즉, useState, onChangeg핸들러, value의 props들이 다 필요없다. <br />
            handleSubmit은 유효성검사 등을 끝내고 onValid, onInvalid 함수를 호출한다. 만약 에러가 나면 그 input으로 focus를 데려간다. <br />
            onValid는 데이터가 유효할 때 호출되는 필수 함수이고, onInvalid은 데이터가 유효하지 않을 때 호출되는 함수로 필수는 아니다.
          </div>
        </div>
    </div>
    )
